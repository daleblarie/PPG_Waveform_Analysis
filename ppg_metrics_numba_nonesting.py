...
@jit(nopython = True)
def calculate_PVI(pleth_signal, window=500, step=10, PI_min=None, PI_max=None, PLOTTING=False):
    ...
    while start_index + window < len(pleth_signal):
        ...
        current_PI = calculate_PI(window_signal)
...
def calculate_resp_rate(pleth_signal, sampling_freq=62.4725, PLOTTING=False):
...
def calculate_PSD(pleth_signal, sampling_freq=62.4725, PLOTTING=False):    # calculating power spectral density
    resp_freq_low = 0.1 #hz (6 breaths/min)
    resp_freq_high = 0.4 #hz (24 breaths/min)
...
# @jit(nopython = True)
def calculate_entropy(pleth_signal):
